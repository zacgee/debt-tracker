...
function avalancheMethod(debts, extraPayment) {
    ...
    while (debts.some(debt => debt.balance > 0)) {
        ...
        for (let debt of debts) {
            ...
            let payment = debt.min;
            if (debt === debts[0]) {
                payment += extraPayment; // Apply extra payment to highest interest debt
            }
...
// Example usage of avalanche method:
const avalancheSchedule = avalancheMethod(debts, extraPayment);

console.log("\nDebt Repayment Schedule (Avalanche Method):");
avalancheSchedule.forEach(record => {
    console.log(`Month ${record.month}:`);
    record.payments.forEach(payment => {
        console.log(`  Paid $${payment.payment} towards ${payment.name}, Remaining Balance: $${payment.remainingBalance.toFixed(2)}`);
    });
});
console.log(`Total Months to Pay Off All Debts: ${avalancheSchedule.length}`);
console.log(`Total Amount Paid: $${avalancheSchedule.reduce((total, record) => total + record.payments.reduce((sum, p) => sum + p.payment, 0), 0).toFixed(2)}`);
console.log(`Total Interest Paid: $${avalancheSchedule.reduce((total, record) => total + record.payments.reduce((sum, p) => {
    const debt = debts.find(d => d.name === p.name);
    const interest = (debt.rate / 12) * (p.remainingBalance + p.payment);
    return sum + interest;
}, 0), 0).toFixed(2)}`);